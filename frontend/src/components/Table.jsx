        ...
        <thead className="bg-bg-elev">
          <tr>
            {columns.map((col) => (
              <th
                key={col.key}
                className={`text-left px-3 sm:px-4 py-2.5 sm:py-3 font-medium text-gray-300 whitespace-nowrap ${col.className || ''}`}
        ...
        <tbody>
          {rows.length === 0 ? (
            ...
            rows.map((row, idx) => (
              ...
              >
                {columns.map((col) => (
                  <td key={col.key} className={`px-3 sm:px-4 py-2.5 sm:py-3 text-gray-200 ${col.cellClassName || ''}`}>
